"""
Performs test simulations of the DQN agent with different settings.
"""
# basic imports
import numpy as np
...
Env = Literal["Topology", "Topology-Dict", "Topology-List"]
...
def dict_obs(nodes: dict[NodeID, Node]) -> tuple[dict[Pose, Observation], Tuple]:
    """
    ...
    space = Tuple([Box(0., 1., (6, )), Box(0., 1., (6, ))])

    return obs, space


def list_obs(nodes: dict[NodeID, Node]) -> tuple[dict[Pose, Observation], Dict]:
    """
    Prepares multimodal observations for the OfflineSimulator from topology nodes.
    ...
    obs: dict[Pose, Observation] = {}
    for _, node in nodes.items():
        obs[node["pose"]] = [np.array(node["pose"]), np.array(node["pose"])]
...
def simulation(
    ...
    elif env_type == "Topology-List":
        simulator = OfflineSimulator(*list_obs(nodes))
...
def main() -> None:
    ...
    The main function.
    """
    env: list[Env] = ["Topology", "Topology-Dict", "Topology-List"]
    policy = [True, False]
    target_update = [0.001, 10]
    for combination in product(env, policy, target_update):
