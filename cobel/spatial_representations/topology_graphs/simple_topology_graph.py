...
class AbstractTopologyGraph(SpatialRepresentation) : 
    ...
    def get_nodes(self) -> list:
        '''
        This function returns the nodes of the topology graph.
        ...
        Returns
        ----------
        nodes :                             The list containing topology graph's nodes.
        '''
        return self.nodes
    
    def get_edges(self) -> list:
        '''
        This function returns the edges of the topology graph.
        ...
        Returns
        ----------
        nodes :                             The list containing topology graph's edges.
        '''
        return self.edges
    ...
    def set_visual_debugging(self):
        ...
        None
        '''
        raise NotImplementedError('.set_visual_debugging() function not implemented!')
    ...
    def get_action_space(self) -> gym.spaces.Discrete:
        ...
        '''
        raise NotImplementedError('.get_action_space() function not implemented!')
    ...
    def generate_behavior_from_action(self, action: int) -> dict:
        ...
        '''
        raise NotImplementedError('.generate_behavior_from_action() function not implemented!')
    ...
    def sample_state_space(self):
        ...
        None
        '''
        raise NotImplementedError('.sample_state_space() function not implemented!')
...
class GridGraph(AbstractTopologyGraph): 
    ...
    def __init__(self, n_nodes_x=5, n_nodes_y=5, n_neighbors=4, x_limits=[-1,1], y_limits=[-1,1], start_nodes=None, goal_nodes=None,
        ...
        if self.world_module is not None: 
            self.world_limits = self.world_module.get_limits()
            self.world_nodes, self.world_edges = self.world_module.get_wall_graph()
        ...
        if self.goal_nodes is not None:
            for node_idx in self.goal_nodes:
                self.nodes[node_idx].goal_node = True
    
    @staticmethod
    def find_grid_points(n_nodes_x: int, n_nodes_y: int, x_limits: np.ndarray, y_limits: np.ndarray) -> np.ndarray:
        '''
        ...
        Returns
        ----------
        grid_points :                       The coordinates for all nodes.
        '''
    ...
    def build_graph(self, grid_points: np.ndarray):
        '''
        This function builds the actual nodes (type : topologynode object) and edges and
        ...
        Parameters
        ----------
        grid_points :                       The x and y coordinates of nodes in the grid.
    ...
    def set_rotation(self, rotation: bool):
        '''
    ...
    def update_position_marker(self, pose: np.ndarray):
        '''
        ...
        None
        '''
        self.position_marker.set_data(pose[0], pose[1], np.rad2deg(np.arctan2(pose[3], pose[2])))
        ...
        if not self.rotation :    
            if action != 'reset' : 
                ...
                if node_id != -1 : 
                    ...
                    self.world_module.goal_reached = True
            else : 
            ...
            if action != 'reset':
                heading = np.array([self.world_module.env_data['pose'][2], self.world_module.env_data['pose'][3]])
    ...
    def calculate_angles_edges(self, node: TopologyNode, heading: np.ndarray, threshold=5) -> (list, list, list, list):
        ...
        Returns
        ----------
        directions :                        The list of angles.
        forward_edge :                      The list of forward edges.
        left_edges :                        The list of left edges.
        right_edges :                       The list of right edges.
        '''
        left_edges, right_edges, forward_edge, directions = [], [], [], []        
        for n in node.neighbors : 
            if n.index != -1 : 
                current_node_pos = np.array([node.x, node.y])
                ...
                directions.append([n.index, edge_vector, world_angle])
                angle = np.arctan2(heading[0]*edge_vector[1] - heading[1]*edge_vector[0],
                                   heading[0]*edge_vector[0] + heading[1]*edge_vector[1])
                angle = angle / np.pi*180.0
                if angle < -threshold:
                    right_edges.append([n.index, edge_vector, angle])
                    left_edges.append([n.index, edge_vector, (360.0+angle)])
                if angle > threshold:
                     left_edges.append([n.index, edge_vector, angle])
                     right_edges.append([n.index, edge_vector, -(360.0-angle)])
    ...
    def init_visual_elements(self):
        ...
        if self.visual_output:
            ...
            self.position_marker = CogArrow(angle=0.0, headLen=20.0, tipAngle=25.0, tailLen=0.0, brush=(255, 0, 0))
            self.plot.addItem(self.position_marker)
            # initial position to center, this has to be worked over later!
            self.position_marker.set_data(0.0, 0.0, 0.0)
            ...
            if self.visual_output:
                ...
                for node in self.nodes:
                    ...
                    if node.index != -1:
                        ...
                        max_neigh_node = node.neighbors[np.argmax(q_values[0:valid_index+1])]
    ...
    def move_to_node(self, node_id: int, angle=90.0):
        ...
        self.world_module.actuate_robot(np.array([next_node_pos[0], next_node_pos[1], angle]))
        self.current_node = self.next_node
        self.observation_module.update()
        if self.visual_output:
            self.update_position_marker([next_node_pos[0], next_node_pos[1], np.cos(np.deg2rad(angle)), np.sin(np.deg2rad(angle))])
...
class HexagonalGraph(GridGraph):
    '''
    Hexagonal grid topology graph class.
    ...
    def calculate_angle(node_info: TopologyNode, neighbor_info: TopologyNode) -> float:
        ...
        Parameters
        ----------
        node_info :                         The first node.
        neighbor_info :                     The second node.
    ...
    def sort_graph(self, node_info: TopologyNode):
        '''
        This function sorts neighbors of a specified according to their angle.
    ...
    def build_graph(self, grid_points: np.ndarray):
        '''
        This function builds the topology graph.
        ...
        for i, p in enumerate(mesh.points):
            ...
            self.nodes.append(TopologyNode(i, p[0], p[1]))
        ...
        for edge in self.edges:
            # first and second edge nodes
            a, b = self.nodes[int(edge[0])], self.nodes[int(edge[1])]
