...
def single_run(visual_output: bool = True):
    ...
    if visual_output:
        main_window = qg.GraphicsLayoutWidget(title='Demo: PMA Agent')
        main_window.show()
    
    # initialize world            
    world = make_open_field(1, 10, 9, 10)
    ...
    # initialize RL agent
    rl_agent = PMAAgent(interface_OAI=modules['rl_interface'], policy=policy, policy_test=policy,
