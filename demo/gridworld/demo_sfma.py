"""
A demo simulation that trains a Dyna-Q agent which uses SFMA to replay
...
from cobel.memory.utils import DR
...
def simulation() -> None:
    ...
    steps: int = 50
    # prepare widget
    main_window = GraphicsLayoutWidget(title='Demo: SFMA Agent')
    main_window.show()
    # init environment
    invalid_transitions = [
        (3, 4),
    ...
    memory = SFMAMemory(metric, env.world['states'], 4)
    agent = SFMA(
