...
class Model(torch.nn.Module):
    ...
    def __init__(self, input_size: int | tuple, number_of_actions: int):
        ...
        self.layer_dense_1 = torch.nn.Linear(in_features=input_size, out_features=64)
        self.layer_dense_2 = torch.nn.Linear(in_features=64, out_features=64)      
        self.layer_output = torch.nn.Linear(in_features=64, out_features=number_of_actions)
        self.double()
...
def single_run(visual_output: bool = True):
    ...
    if visual_output:
        main_window = qg.GraphicsLayoutWidget(title='Demo: MFEC with pretrained model & Discrete Interface')
        main_window.show()
    ...
    # initialize RL agent
    rl_agent = MFECAgent(modules['rl_interface'], policy, pretrained_model=TorchNetwork(Model(25, 256)), custom_callbacks={'on_trial_end': [reward_monitor.update]})
