...
# CoBel-RL framework
from cobel.agents.dqn import SimulatingDQN
...
class Model(torch.nn.Module):
    ...
    def __init__(self, input_size: int | tuple, number_of_actions: int, sigmoid: bool = False):
        super().__init__()
...
class ModelTransition(torch.nn.Module):
...
def single_run(visual_output: bool = True):
    ...
    # the internal model of the agent
    model_transition = FlexibleTorchNetwork(ModelTransition(29, 25))
    model_reward = TorchNetwork(Model(25, 1))
    model_terminal = TorchNetwork(Model(25, 1, True))
    internal_model = InternalModel(4, np.copy(observations[20:21]), model_transition, model_reward, model_terminal, 10000, 128, 1)

    # amount of trials
    trials_train = 600
    trials_simulate = 100
    trials_test = 100
    number_of_trials = trials_train + trials_simulate + trials_test
    ...
    # initialize RL agent
    rl_agent = SimulatingDQN(modules['rl_interface'], internal_model, policy, policy, 0.8,
                             TorchNetwork(Model((25,), 4)), {'on_trial_end': [reward_monitor.update]})
    ...
    rl_agent.train(trials_simulate, max_steps, simulate=True)
