    ...
    if logs['trial'] % 300 == 0:
        ...
        while validityCheck == False:
            ...
            for barrier in barrierIDs:
                ...
                texture = '../../environments/environments_blender/textures/wall_01.bmp'
                logs['rl_parent'].interface_OAI.modules['world'].set_barrier(barrier, np.random.rand() < barrierProbability, 0, texture)
...
def single_run(visual_output: bool = True):
    ...
    if visual_output:
        main_window = qg.GraphicsLayoutWidget(title='Demo: Dynamic Barriers')
        main_window.show()
    
    # determine demo scene path
    demo_scene = '../../environments/environments_blender/simple_grid_graph_dynamic.blend'
    ...
    modules['spatial_representation'] = ManualTopologyGraphNoRotationDynamic(modules,{'start_nodes':[0], 'goal_nodes':[15], 'clique_size':4})
