...
def single_run():
    ...
    if visual_output:
        main_window = qg.GraphicsWindow(title='Demo: Offline')
    
    # determine framework path
    path = os.path.abspath(__file__).split('cobel')[0] + '/cobel/'
    # determine world info file paths
    imageSet = path + 'demo/offline_demo/worldInfo/images.npy'
    safeZoneDimensions = path + 'demo/offline_demo/worldInfo/safeZoneDimensions.npy'
    safeZoneVertices = path + 'demo/offline_demo/worldInfo/safeZoneVertices.npy'
    safeZoneSegments = path + 'demo/offline_demo/worldInfo/safeZoneSegments.npy'
