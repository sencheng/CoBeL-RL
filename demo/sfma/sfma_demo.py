...
def single_run(visual_output: bool = True):
    ...
    if visual_output:
        main_window = qg.GraphicsLayoutWidget(title='Demo: SFMA Agent')
        main_window.show()
    ...
    # initialize RL agent
    rl_agent = SFMAAgent(interface_OAI=modules['rl_interface'], policy=policy, policy_test=policy, learning_rate=0.9,
                        gamma=0.9, gamma_SR=0.9, custom_callbacks={'on_trial_end': [escape_latency_monitor.update]})
