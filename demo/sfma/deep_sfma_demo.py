...
# framework imports
from cobel.agents.sfma import DeepSFMAAgent
from cobel.policy.softmax import SimpleSoftmax
...
def single_run(visual_output: bool = True):
    ...
    if visual_output:
        main_window = qg.GraphicsLayoutWidget(title='Demo: Deep SFMA Agent')
        main_window.show()
    ...
    # define policy
    policy = SimpleSoftmax(5)
    
    # initialize RL agent
    rl_agent = DeepSFMAAgent(interface_OAI=modules['rl_interface'], policy=policy, policy_test=policy, gamma=0.9,
                             gamma_SR=0.1, model=model, custom_callbacks={'on_trial_end': [escape_latency_monitor.update]})
