        ...
        if self.n_step > 1:
            for i in range(self.batch_size):
                ...
                for j in range(1,self.n_step):
                    next_index = (indices[i]+j) % self.__len__()
                    if self.done_buf[next_index]:
                        break
                    else:
                        rews[i] = rews[i] + self.rews_buf[next_index] * self.gamma**j
                        next_obs[i] = self.next_obs_buf[next_index]
                        done[i] = self.done_buf[next_index]
